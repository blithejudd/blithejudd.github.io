...
- `css/site.css` — тёплая editorial-тема, адаптивная сетка, клавиатурный фокус, reduced motion.
...
- `admin/index.html`, `css/admin.css`, `js/admin.js` — русская панель владельца; редактор, очередь, подтверждение удаления.
